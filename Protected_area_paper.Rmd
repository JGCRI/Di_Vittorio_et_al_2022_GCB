...
write.csv(landuse.d , paste0(output_path_csv,"5. Change in land allocation in 2050 across constraints ","all_LT",".csv"))
...
data_for_plot %>% left_join(low_avail_details) %>% mutate(available= available) %>% 
  ...
  mutate(new_unmanaged=if_else(is.na(new_unmanaged),needed_managed,new_unmanaged))->data_for_plot
...
g <- ggplot()+
   geom_point(data=intersection_points %>% na.omit() %>%  
             ...
             aes(x=available,y=value,color=agg.land),size=5)+
  geom_point(data=intersection_points_needed %>% filter(region==region_select,!agg.land %in% c("rock and desert", "urban","desert")),aes(x=new_unmanaged,y=value,color=agg.land),show.legend = FALSE,shape=17,size=5) +
  ...
  scale_shape_manual(values=c(0,1,2))+scale_x_continuous(labels=formatter1000,breaks = seq(0,100,10))
...
## Figure 7: Marginal change in land allocation vs Marginal change in availability (Percent)
```{r,fig.width=10,fig.height=10}
...
g <- ggplot()+
  ...
  geom_vline(data=data_for_plot %>% na.omit() %>% filter(region==region_select),aes(xintercept=available,color=agg.land), linetype = 3,size=2,show.legend = FALSE) +
  facet_wrap(~region,scales="free_y")+
  ylab("Marginal percent increase/decrease in Percent")+
  xlab("Availability")+
  ggtitle("Marginal increase/decrease in land allocation by land type in 2050 with increase in protection")+
  scale_color_lancet()+
  guides(color=guide_legend(title="Land type"))+
