...
# Load libraries
```{r, libraries& initializations,echo=FALSE, warning=FALSE}
library(rgcam)
library(dplyr)
library(tidyr)
library(ggplot2)
...
```{r, Land allocation plot, fig.height=6, fig.width=10,echo=FALSE, message=FALSE, warning=FALSE}
...
landuse.d[landuse.d[,"land-allocation"] == "biomassGrass", "agg.land"] <- "(a) Biomass"
landuse.d[landuse.d[,"land-allocation"] == "biomassTree", "agg.land"] <- "(a) Biomass"
...
ggsave( paste0("outputs/images/land_allocation_plots/",'1. Land allocation by type ',project_name,toString(i),'.png'),width = 10.5, height = 10)
}
...
```{r, crop production, fig.width=10, fig.height=6,echo=FALSE,message=FALSE, warning=FALSE}
dir.create("outputs/images/crop_production_plots/")

crop_prod_mapping <- read.csv("other_data/Crop_prod_mapping.csv", stringsAsFactors = FALSE)
...
crop <- bind_rows(new_table) %>% 
        ...
        left_join(crop_prod_mapping) %>% 
        dplyr::select(-output) %>% 
        rename(output=output_name)
...
fuel_mapping <- read.csv("other_data/fuel_mapping.csv", stringsAsFactors = FALSE)
...
energy_master <- bind_rows(energy_master, energy_master_total) %>% 
                 ...
                 left_join(fuel_mapping) %>% 
                 dplyr::select(-fuel) %>% 
                 rename(fuel=fuel_name)
...
g <- ggplot()+
  ...
  labs(subtitle = "Dots are the starting percent of available land in 2015.Triangles are new percentage of available land after implementing 30 by 30 targets.", size=5)
...
```
## Figure 6: Marginal change in land allocation vs Marginal change in availability (thous km2) all regions
```{r,fig.height=14,fig.width=10,echo=FALSE,message=FALSE, warning=FALSE}
g <- ggplot()+
   geom_point(data=intersection_points %>% na.omit() %>%  
...
```
#Figure 6: Marginal change in land allocation vs Marginal change in availability (Percent) all regions
```{r,fig.height=14,fig.width=10,echo=FALSE,message=FALSE, warning=FALSE}
